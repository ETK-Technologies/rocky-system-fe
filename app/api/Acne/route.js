...
async function getEntrykey() {
    try {
        ...
        const existingCookie = cookieStore.get("acne_entrykey");
        return (
            existingCookie?.value || `acne-${randomBytes(8).toString("hex")}`
        );
    } catch (error) {
        logger.warn("Cookie reading error:", error);
        return `acne-${randomBytes(8).toString("hex")}`;
    }
}
...
export async function GET() {
    ...
    response.cookies.set("acne_entrykey", entrykey, {
        domain: "myrocky.com",
        path: "/",
...
export async function POST(req) {
    try {
        ...
        const data = {
            form: {},
            acne_entrykey: entrykey,
            timestamp: Math.floor(Date.now() / 1000),
            entry_saved: false,
            form_id: rawData.form_id || 11,
            error: false,
            id: rawData.id || "",
            token: rawData.token || "",
