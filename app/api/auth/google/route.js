...
export async function GET(req) {
  try {
    ...
    stateParts.push(
      `frontend_callback:${encodeURIComponent(frontendCallbackUrl.toString())}`
    );
    ...
    // Backend MUST use redirect_uri: {BASE_URL}/api/v1/auth/google/callback
    ...
    return NextResponse.redirect(
      new URL(
        "/login-register?viewshow=login&error=oauth_init_failed",
        req.nextUrl.origin
      )
    );
  }
