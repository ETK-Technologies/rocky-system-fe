...
async function getEntrykey() {
    try {
        ...
        const existingCookie = cookieStore.get("hyperpigmentation_entrykey");
        return (
            existingCookie?.value || `hyper-${randomBytes(8).toString("hex")}`
        );
    } catch (error) {
        logger.warn("Cookie reading error:", error);
        return `hyper-${randomBytes(8).toString("hex")}`;
    }
}
...
async function getUserDataFromCookies() {
    try {
        ...
        logger.log("Hyperpigmentation API - Final user data:", userData);
        return userData;
...
export async function GET() {
    ...
    response.cookies.set("hyperpigmentation_entrykey", entrykey, {
        domain: "myrocky.com",
        path: "/",
...
export async function POST(req) {
    try {
        ...
        const data = {
            form: {},
            hyperpigmentation_entrykey: entrykey,
            timestamp: Math.floor(Date.now() / 1000),
            entry_saved: false,
            form_id: rawData.form_id || 9,
            error: false,
            id: rawData.id || "",
            token: rawData.token || "",
...
async function postToCRM(data) {
    ...
    const postData = {
        ...
        hyperpigmentation_entrykey: data.hyperpigmentation_entrykey || "",
    };
