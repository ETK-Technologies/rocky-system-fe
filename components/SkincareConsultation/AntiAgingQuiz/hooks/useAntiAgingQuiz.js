...
export function useAntiAgingQuiz() {
  // Local storage + background queue (mirrors hair questionnaire resilience)
  const storageKey = "quiz-form-data-antiaging";
  const storageExpiryKey = "quiz-form-data-antiaging-expiry";
  ...
  const metaRef = useRef({ id: "", token: "", antiaging_entrykey: "" });
  ...
  // Initialize entrykey cookie and restore any saved progress
  useEffect(() => {
    (async () => {
      try {
        await fetch("/api/AntiAging", {
          method: "GET",
          credentials: "include",
        });
      } catch (e) {
        logger.warn("Failed to prefetch antiaging entrykey", e);
      }
  ...
  // Track last submitted question to prevent duplicate submissions
  ...
  const enqueueSubmission = (payload, skipDuplicateCheck = false) => {
    ...
    if (Object.keys(clean).length === 1 && !skipDuplicateCheck) {
      ...
      // Skip if we've already submitted this exact answer for this question
  ...
  const processQueue = async () => {
    ...
    try {
      ...
      if (result && (result.id || result.token || result.antiaging_entrykey)) {
        metaRef.current = {
          id: result.id || metaRef.current.id,
          token: result.token || metaRef.current.token,
          antiaging_entrykey:
            result.antiaging_entrykey || metaRef.current.antiaging_entrykey,
        };
        writeLocalStorage({
          answers,
          stepIndex,
      ...
      logger.error("AntiAging sync error:", e);
    } finally {
  ...
  const submitFormData = async (specificData = null, cleanedAnswers = null) => {
    try {
      const essentialData = {
        form_id: quizConfig.form_id || 8,
        action: "skincare_antiaging",
        antiaging_entrykey: metaRef.current.antiaging_entrykey || "",
      ...
      const response = await fetch("/api/AntiAging", {
        method: "POST",
      ...
      metaRef.current = {
        ...
        antiaging_entrykey:
          data.antiaging_entrykey || metaRef.current.antiaging_entrykey,
      };
      writeLocalStorage({
        answers,
        stepIndex,
      ...
      logger.error("Error submitting antiaging form:", error);
      return null;
    }
  };
  ...
  const handleBack = () => {
    // Custom logic for AntiAgingQuiz
    let targetStep;
    if (stepIndex === 8) {
  ...
  const transformAnswersForApi = (ans) => {
    ...
    Object.values(steps).forEach((cfg) => {
      ...
      if (cfg.type === "checkbox") {
        ...
        // For debugging: if selectedOption.showTextInput is false/undefined, textFieldValue is ignored
