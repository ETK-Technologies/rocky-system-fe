...
function getOriginFromRequest(requestOrHeaders) {
  ...
  if (requestOrHeaders) {
    ...
    if (requestOrHeaders.headers?.get) {
      origin = requestOrHeaders.headers.get("origin") || requestOrHeaders.headers.get("referer") || requestOrHeaders.headers.get("host");
    }
    // If it's a Headers object
    else if (requestOrHeaders.get) {
      origin = requestOrHeaders.get("origin") || requestOrHeaders.get("referer") || requestOrHeaders.get("host");
    }
...
export async function fetchProductBySlugFromBackend(slug, includeMeta = false, requestOrHeaders = null) {
  ...
  try {
    ...
    const response = await axios.get(url, {
      ...
      validateStatus: function (status) {
        ...
        if (!isValid && process.env.NODE_ENV === "development") {
          console.warn(`[Product API] Received unexpected status code: ${status}`);
        }
        return isValid;
      },
    ...
    if (error.response) {
      ...
      logger.error(`[Product API] ${status} error response for slug "${slug}":`, {
        status,
        data: errorData,
      ...
      if (status === 401) {
        logger.error(`[Product API] 401 Unauthorized - Origin header: ${getOriginFromRequest(requestOrHeaders)}`);
      }
    } else if (error.request) {
      logger.error(`[Product API] No response received for slug "${slug}":`, error.message);
    } else {
      logger.error(`[Product API] Error fetching product "${slug}":`, error.message);
    }
